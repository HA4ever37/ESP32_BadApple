...
Adafruit_SH1106 display(21, 19, 22);
...
void readFile(fs::FS &fs, const char * path) {
  ...
  if (!file || file.isDirectory()) {
    ...
    display.print("File open error. Upload video.hs using ESP32 Sketch Upload.");
    display.display();
...
void setup() {
  ...
  if (!SPIFFS.begin()) {
    ...
    display.print("SPIFFS mount failed. Upload video.hs using ESP32 Sketch Upload.");
    display.display();
    return;
  }
